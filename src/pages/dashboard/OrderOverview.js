...
import { Card, Col, Container, Row, Table, Button } from "@themesberg/react-bootstrap";
            ...
            <Card>
              <Card.Body>
                <div className="d-flex align-items-center justify-content-between my-3">
                  ...
                  <Button variant="secondary" className="m-1 text-white" onClick={() => {createOrder()}}>+ Create new order</Button>
                </div>
                <Table>
                  <thead className="">
                    <tr>
                  ...
                  <tbody>
                    {orderData.map((item, index) => (
                      <tr key={`order-${index}`}>
                        ...
                        <td>{!item.active ? "Active" : "Canceled"}</td>
                        <td>{item.power | 0}GH/s</td>
                        <td>{item.payAmount}</td>
                        <td>{item.created_at}</td>
                        <td>{moment(item.created_at).add(30, "minutes").format('YYYY-MM-D h:mm')}</td>
                        <td></td>
                      </tr>
                    ))}
                  </tbody>
